...
function StepBar({ current }: { current: number }) {
  return <nav aria-label="Chapter progress" style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "15px", gap: 5, background: "var(--bg-surface)", borderBottom: "1px solid var(--bg-border)", position: "sticky", top: 0, zIndex: 10 }}>
    {STEPS.map((name, index) => <div key={name} style={{ display: "flex", alignItems: "center", gap: 5 }}>{index > 0 && <div style={{ width: 28, height: 2, background: index <= current ? "var(--accent-gold)" : "var(--bg-border)" }} />}<div style={{ border: `1px solid ${index === current || index < current ? "var(--accent-gold-border)" : "var(--bg-border)"}`, background: index === current ? "var(--accent-gold-glow)" : "transparent", color: index <= current ? "var(--accent-gold)" : "var(--text-muted)", borderRadius: 999, padding: "5px 12px", fontFamily: "var(--font-body)", fontSize: 11, fontWeight: index === current ? 700 : 500 }}>{index < current ? "✓ " : ""}{name}</div></div>)}
  </nav>;
}
...
export default function HalfYearlyChapterPage() {
  ...
  const pad = isMobile ? "20px 16px 100px" : isTablet ? "30px 28px" : "38px 48px";
  return <main style={{ minHeight: "100vh", background: "var(--bg-base)" }}>
    <StepBar current={step} />
    <div style={{ maxWidth: 980, margin: "0 auto", padding: pad, boxSizing: "border-box" }}>
      <button onClick={() => router.push(`/dashboard/half-yearly/${subjectKey}`)} style={backStyle}>← {subject.label}</button>
      <header style={{ margin: "14px 0 27px" }}><div style={{ color: subject.color, fontSize: 11, fontFamily: "var(--font-body)", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase" }}>{subject.label} · Chapter {chapterIndex + 1}</div><h1 style={{ color: "var(--text-primary)", fontFamily: "var(--font-display)", fontSize: isMobile ? 28 : 38, letterSpacing: "-.035em", margin: "8px 0 0" }}>{chapter.title}</h1></header>
      ...
      {step === 0 && <section>
        ...
        {videos.length > 0 && <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, 1fr)", gap: 10, marginTop: 14 }}>{videos.map(video => <button key={video.videoId} onClick={() => setActiveVideo(video.videoId)} style={{ cursor: "pointer", display: "flex", textAlign: "left", padding: 8, gap: 10, background: activeVideo === video.videoId ? `${subject.color}16` : "var(--bg-surface)", border: `1px solid ${activeVideo === video.videoId ? subject.color : "var(--bg-border)"}`, borderRadius: 10, color: "inherit" }}><img src={video.thumbnail} alt="" width="98" height="55" style={{ objectFit: "cover", borderRadius: 6 }} /><span style={{ minWidth: 0 }}><span style={{ display: "block", color: "var(--text-primary)", fontFamily: "var(--font-body)", fontSize: 12, lineHeight: 1.35 }}>{video.title}</span><span style={{ display: "block", color: "var(--text-muted)", fontSize: 10, marginTop: 4 }}>{video.channelTitle}</span></span></button>)}</div>}
      ...
      {step === 1 && <section>
        ...
        <div ref={readerRef} style={{ background: "var(--bg-surface)", border: "1px solid var(--bg-border)", borderRadius: 14, overflow: "hidden", marginTop: 18 }}>
          ...
          <div style={{ height: isMobile ? 440 : 610, overflow: "auto", display: "grid", placeItems: "start center", background: "#17171e", padding: 12 }}>
