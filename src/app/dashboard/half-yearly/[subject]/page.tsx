...
export default function HalfYearlyChaptersPage() {
  ...
  return <main style={{ minHeight: "100vh", background: "var(--bg-base)", padding: isCompact ? "24px 18px 100px" : "48px", boxSizing: "border-box", minWidth: 0 }}>
    ...
    <div style={{ maxWidth: 820, margin: "0 auto" }}>
      ...
      <header style={{ display: "flex", alignItems: "center", gap: 14, margin: "10px 0 30px", minWidth: 0 }}>
        <span style={{ fontSize: isCompact ? 34 : 42, flexShrink: 0 }}>{subject.icon}</span><div style={{ minWidth: 0 }}><h1 style={{ margin: 0, color: "var(--text-primary)", fontFamily: "var(--font-display)", fontSize: isCompact ? 27 : 36, letterSpacing: "-.03em", overflowWrap: "anywhere" }}>{subject.label}</h1><p style={{ color: "var(--text-muted)", margin: "5px 0 0", fontSize: 12, fontFamily: "var(--font-body)" }}>{subject.chapters.length} half-yearly chapters · Watch → Read → Test</p></div>
      </header>
      <div style={{ display: "grid", gap: 10 }}>
        {subject.chapters.map((chapter, index) => {
          ...
          return <button key={chapter.id} onClick={() => router.push(`/dashboard/half-yearly/${subjectKey}/${chapter.id}`)} onMouseEnter={() => setHovered(chapter.id)} onMouseLeave={() => setHovered(null)} style={{ minWidth: 0, textAlign: "left", cursor: "pointer", color: "inherit", background: active ? "var(--bg-elevated)" : "var(--bg-surface)", border: `1px solid ${complete ? "var(--accent-gold-border)" : active ? `${subject.color}75` : "var(--bg-border)"}`, borderRadius: 14, padding: isCompact ? "14px" : "17px 20px", display: "flex", alignItems: "center", gap: 14, transform: active ? "translateX(4px)" : "none", transition: "all .2s ease", animation: `hy-list .3s ease-out ${index * 35}ms both` }}>
            ...
            <span style={{ flex: 1, minWidth: 0 }}><span style={{ display: "block", color: "var(--text-primary)", fontFamily: "var(--font-display)", fontSize: isCompact ? 14 : 16, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{chapter.title}</span><span style={{ display: "block", marginTop: 4, color: complete ? "var(--accent-gold)" : inProgress ? subject.color : "var(--text-muted)", fontFamily: "var(--font-body)", fontSize: 11 }}>{complete ? "Completed" : inProgress ? `Step ${entry.step} of 3` : "Not started"}{!chapter.precisionChapter && " · guided reading"}</span></span>
